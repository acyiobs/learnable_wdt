...
class SolverPathsWrapper(SolverPaths):
    def _spec_transition_matrices_interaction(self, paths, paths_tmp, scattering):
        ...
        # [max_depth, num_targets, num_sources, max_num_paths]
        transfer_mtx = self._scene.neural_scene(objects, vertices, theta_, 0)
    ...
    def _diffraction_transition_matrices_interaction(self, paths, paths_tmp):
        ...
        geo_feat = tf.stack([phi_prime_, phi_, beta_prime, beta, n_, s_prime_, s_], -1)

        # [max_depth, num_targets, num_sources, max_num_paths]
        mat_no_scale = self._scene.neural_scene(
            objects_indices[..., 0], diff_points, geo_feat, 1
        )
    ...
    def _compute_paths_coefficients_interaction(
        ...
        if n_scat > 0:
            ...
            geo_feat = tf.concat([a_in_all, scat_k_i, scat_k_s, scat_normal], -1)
            geo_feat = tf.squeeze(geo_feat, axis=[1, 2, 3])

            valid_object_idx = tf.squeeze(valid_object_idx, axis=[1, 2, 3])
            scat_points = tf.squeeze(scat_points, axis=[1, 2, 3])

            a_scat_out = self._scene.neural_scene(
                valid_object_idx, scat_points, geo_feat, 2
            )
